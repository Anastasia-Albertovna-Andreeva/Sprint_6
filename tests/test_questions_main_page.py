...
class TestQuestionsMainPage:
    ...
    @allure.description('Кликаем на вопросы по порядку и проверяем ответы в выпадающем списке')
    ...
    def test_questions(self, driver, question_key, expected_answer):
        ...
        main_page.scroll_to_faq_section()  # Прокручиваем до секции вопросов

        # Ждем, пока вопрос станет видимым, и кликаем на вопрос
        main_page.wait_visibility_of_faq_item(question_key)
        main_page.click_on_faq_item(question_key)

        # Ждем, пока ответ станет видимым и будет текст ответа
